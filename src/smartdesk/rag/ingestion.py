...
Sources (real + synthetic combined):
  - HR: strova-ai/hr-policies-qa-dataset from Hugging Face
  - IT/HR: local .txt and .json files from data/knowledge_base/{it,hr}_docs/
           (generated by scripts/generate_synthetic_docs.py or added manually)
  - IT: optional HuggingFace dataset — configure IT_DATASET_NAME in load_huggingface_it()
"""
...
def load_huggingface_it() -> List[Document]:
    ...
    Set IT_DATASET_NAME to a HuggingFace dataset identifier to supplement the
    synthetic IT docs generated by scripts/generate_synthetic_docs.py.
    Leave empty to rely on synthetic IT docs only (the current default).
    """
    IT_DATASET_NAME = ""  # e.g. "KameronB/synthetic-it-callcenter-tickets"

    if not IT_DATASET_NAME:
        print("[ingestion] No HF IT dataset configured — using synthetic IT docs only.")
        return []
...
def load_all_sources() -> List[Document]:
    """Load IT + HR documents from all sources (real + synthetic).

    Execution order:
      1. HF HR dataset  (strova-ai/hr-policies-qa-dataset)
      2. HF IT dataset  (optional — configure IT_DATASET_NAME in load_huggingface_it)
      3. Local HR files (data/knowledge_base/hr_docs/)
      4. Local IT files (data/knowledge_base/it_docs/)
    """
    docs: List[Document] = []
